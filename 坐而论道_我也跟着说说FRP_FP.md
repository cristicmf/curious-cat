...
说响应式编程之前,得先知道啥是响应式.这个概念是什么,哪里来的,怎么来的.
据我个人了解(道听途说),响应式得从响应式宣言开始. 
就像共产党有马克思的<<共产主义宣言>>,美国有<<独立宣言>>.
要不然,独立自由,共产主义这些词怎么来的呢? 同样,响应式也有响应式宣言. 
传送门: https://www.reactivemanifesto.org/
中文版: https://www.reactivemanifesto.org/zh-CN


读完你就知道, 最后一句话非常重点
`现在,是时候在系统设计一开始就有意识地应用这些设计原则了， 而不是每次都去重新发现它们`
也就是说,这个其实是为了提升系统质量的业界准则. 一个好的系统,至少需要考虑哪些方面.
这个宣言里也总结了一些特性,我就不跟着抄了. 
下面紧跟着的一个概念是响应式系统, 满足这些特性的系统,就是响应式系统.
那紧接着就有一个问题,如何构建这样一个系统? 答案不止一个.
我说两个思路.
1. 使用现有的响应式框架开发一套系统. (从编程层面解决)
2. 使用K8s等容器技术,部署响应式系统. 也就是我开发的应用不是响应式的应用. 但是我通过把应用容器化,使用其他产品,将应用包装.
最后系统整体表现出响应式.

那第一种思路的构建系统的方法,可以给个统一的名字,那就是响应式编程.
也就是说它是在代码层面解决问题.而不是在架构及部署层面来解决.

那FRP呢? 那就是用函数式的编程方法,来构建一个响应式系统.
在FRP里呢,有个非常重要的概念,叫流. 在响应式宣言里,有"消息驱动"的要求,反应式系统依赖异步的消息传递,
用异步消息主要是因为系统有低耦合的需求,而异步消息可以很方便的解耦. 如果有其他比较好的解耦的方法,那我个人就可以说,
消息驱动不是必须,当然,好像没有.

流是什么呢? 流就是一串串消息,它源源不断,就像水流一样,一滴滴水滴,汇总起来,成了水流. 一个个消息聚集起来就成了流.

那此时此刻我们可以联想一下流水. 高山流水觅知音,长江后浪推前浪,都江堰,泉水叮咚,碧涧流泉. 这些都是可以联想的东西.
编程也有诗意.哈哈～
联想完回来我们看看处理流,这其实是一个"用水系统",如果你真的见过一个用水系统.
首先,我们对流有几种基本的操作需求. 
1. 分发
    以用水为例,水从上游下来,有工业用水和居民用水的需求. 
    居民用水用水量小,水质要求高.那就得引条小流,去消毒净化,最后流进千家万户.
    工业用水量大,水质要求低,那就可以引条大流,简单处理,然后流进工厂.
    在系统中,有些消息是业务相关的，有些消息是系统相关的. 根据它们不同的主题及消费者,分发给它们.
2. 过滤
    过滤是更细层次的分发.
3. 合并
    以农业用水,灌溉为例,各个田地灌溉完后,需要把这些水，再排到下游去. 
    山里的水要和江里的水汇合到海里去.这就是合并.
4. 转换
    转换是要产生新的流.
    最简单的例子,一杯凉水烧开变成开水.
    受污染的水,经处理后变成可用水. 这些都是转换. 最简单最有趣的转换是什么也不做. 函数式里叫id函数(identify). (x)=>x

这些都是流的需求.
FRP之所以在响应式编程里一枝独秀.主要是因为,它的这些流的处理需求,函数式编程天生可以很好的处理.
1. 分发, 有模式匹配
2. 过滤有过滤函数
3. 合并,函数式编程本来就鼓励组合.
4. 转换,每个函数其实本身就是一个转换器.
总之,种种天赋及基因,让函数式和响应式互相很有眼缘.
这个就是我对FRP的一些个人理解了.

下面我简单介绍一个RxJS的使用. 因为其他响应式的框架啥,我也没用过.
我从我个人对响应式的理解,介绍一下RxJS.
中文文档官网在这里: https://cn.rx.js.org/
它的安装及使用参看文档即可.
如果业务场景简单,它代码其实比较啰嗦的.
但是,对于复杂的业务需求. 使用它就很方便了.
它的模型比较简单,
Observable (生产者)来制造流
subscribe (消费者) 来消费流
它的操作符很溜. 
 merge, concat,forkJoin,mapping,filter,take,takeUntil
 也就是这些操作符,很方便操作流.

这里有一点我必须得说明,一般RxJS用在前端页面或安卓之类的比较多. 后端拿RxJS开发的我个人还没见过.可能后端比较简单吧.
